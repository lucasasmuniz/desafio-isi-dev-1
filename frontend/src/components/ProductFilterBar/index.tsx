...
export default function ProductFilterBar({ onFiltering }: Props) {
  ...
  function handleOnClickFilter(event: any) {
    ...
    if (text === "" && minPrice === "" && maxPrice === "" && hasDiscount === false) {
      return;
    }
    setUseFilter(true);
    onFiltering(parseCurrencyBRL(minPrice) === "0.00" ? "" : parseCurrencyBRL(minPrice), parseCurrencyBRL(maxPrice) === "0.00" ? "" : parseCurrencyBRL(maxPrice), text, hasDiscount);
  }
      ...
      <div className='price-container'>
        ...
        </div>
        {
          ...
          <div className="toggle-container">
            ...
            <span className="toggle-label">Somente com desconto</span>
        </div>
      </div>
